...
function Layout({ onLogout, user }) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      console.log(error);
      setError(
        error.message || "coud not load task , check error in layout.jsx"
      );
  ...
  const StatCard = ({ title, value, icon }) => (
    <div
      className="sm:p-3 rounded-xl bg-white shadow-sm border
    border-purple-100 hover:shadow-md transition-colors
    duration-300
     hover:border-purple-100 group"
    >
      <div className="flex items-center gap-2">
        <div
          className="p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10  group-hover:from-fuchsia-500/20
      ...
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center ">
        <div
          className="bg-red-50 text-red-600
            p-4 rounded-xl border border-e-red-100 max-w-md"
        >
          <p className="font-medium mb-2">error loding tasks</p>
          <p className="text-sm">{error}</p>

          <button
            onClick={fetchTasks}
            className="mt-4 py-2 px-4 bg-red-100
                text-red-700 rounded-lg text-sm *:font-medium
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div
        className="ml-0 xl:ml-64 lg:ml-64 
      pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300 md:ml-16 "
      >
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="xl:col-span-1 space-y-4 sm:space-y-6">
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
              ...
              <div className="space-y-2 sm:spce-y-3">
                <div className="flex items-center justify-between text-gray-700">
                  <span
                    className="text-sx sm:text-sm font-medium
                  flex items-center gap-1.5 "
                  >
                    <Circle
                      className="w-2.5 h-3.5 
                    sm:w-3 sm:h-3 text-purple-500 fill-purple-500"
                    />
                    Task Progress
                  </span>
                  <span
                    className="text-xs bg-purple-200
                  text-purple-700 px-1.5 py-0.5 sm:px-2
                  rounded-full"
                  >
                ...
                <div className="relative pt-1">
                  <div className="lex gap-1.5 items-center">
                    <div
                      className="flex-1 h-2 sm:h-3 bg-purple-100
                              rounded-full
                              overflow-hidden"
                    >
                      <div
                        className="h-full bg-gradient-to-r from-fuchsia-500
                                  to-purple-600
            ...
            >
              ...
              mb-3 sm:mb-4 text-gray-800 flex
              items-center gap-2"
              >
              ...
              <div className="space-y-2 sm:space-y-3">
                {tasks.slice(0, 3).map((task) => (
                  <div
                    ...
                    justify-between p-2 sm:p-3 
                    hover:bg-purple-50/50 rounded-lg
